...
--view admin artiste + nbre places réservées
CREATE OR REPLACE VIEW gestion_evenements.artistesParNbrePlaces AS
SELECT a.id_artiste, a.nom, a.nationalite, COALESCE(SUM(r.nb_tickets), 0) AS "nbs_tickets"
FROM gestion_evenements.artistes a LEFT JOIN gestion_evenements.concerts c ON a.id_artiste = c.artiste
    LEFT JOIN gestion_evenements.evenements e ON c.salle = e.salle AND c.date_evenement = e.date_evenement
    LEFT JOIN gestion_evenements.reservations r ON e.salle = r.salle AND e.date_evenement = r.date_evenement
GROUP BY a.id_artiste, a.nom, a.nationalite
ORDER BY 4;

--view admin event-destival-nb places vendues
CREATE OR REPLACE VIEW gestion_evenements.events_festival AS
SELECT e.date_evenement, e.nom AS "nom_event", e.salle, s.nom AS "nom_salle",
       s.capacite - e.nb_places_restantes AS "nb_tickets_vendus", COALESCE(f.nom, 'Aucun festival') AS "nom_festival"
FROM gestion_evenements.salles s, gestion_evenements.evenements e
    LEFT OUTER JOIN gestion_evenements.festivals f ON e.festival = f.id_festival
WHERE s.id_salle = e.salle
...
SELECT * FROM gestion_evenements.evenementsParArtiste WHERE id_artiste = 1;

SELECT * FROM gestion_evenements.clients WHERE id_client = 1;

SELECT * FROM gestion_evenements.artistesParNbrePlaces;
SELECT * FROM gestion_evenements.events_festival WHERE date_evenement BETWEEN '2025-05-01' AND '2025-05-31';

-------------------------------------------- GRANTS --------------------------------------------
/*GRANT CONNECT ON DATABASE db TO username;
GRANT USAGE, SELECT ON ALL SEQUENCES IN SCHEMA gestion_evenements TO username;
GRANT USAGE ON SCHEMA gestion_evenements TO username;

GRANT SELECT ON gestion_evenements.artistes, gestion_evenements.clients, gestion_evenements.concerts,
    gestion_evenements.evenements, gestion_evenements.festivals, gestion_evenements.reservations,
    gestion_evenements.salles TO username;

GRANT EXECUTE ON ALL FUNCTIONS IN SCHEMA gestion_evenements TO username;

GRANT UPDATE ON ALL TABLES IN SCHEMA gestion_evenements TO username;
GRANT INSERT ON ALL TABLES IN SCHEMA gestion_evenements TO username;*/
